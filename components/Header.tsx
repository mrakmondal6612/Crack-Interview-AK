...
export default function Header({ user }: { user?: { name?: string; profileURL?: string } }) {
  ...
  return (
    <header className="w-full bg-dark-900 shadow-md py-3 px-4 sm:px-6 flex items-center justify-between border-b border-dark-700 sticky top-0 z-50">
      <Link href="/" className="flex items-center gap-2 min-w-0">
        <Image src="/logo.svg" alt="logo" width={32} height={32} style={{ width: "auto", height: "auto" }} />
        <span className="font-bold text-lg text-primary-100 truncate">PrepWise</span>
      </Link>
      ...
      <nav className={`flex-col sm:flex-row sm:flex items-center gap-6 ${menuOpen ? 'flex' : 'hidden'} sm:!flex absolute sm:static top-16 left-0 w-full sm:w-auto bg-dark-900 sm:bg-transparent p-4 sm:p-0 transition-all`}> 
        ...
        {user && (
          <div className="flex items-center gap-2 py-2 sm:py-0">
            {user.profileURL && (
              <Image src={user.profileURL} alt={user.name || "User"} width={32} height={32} className="rounded-full border border-primary-300" />
            )}
            <span className="text-primary-100 font-medium truncate max-w-[100px]">{user.name}</span>
          </div>
        )}
